...
//#region node_modules/.nitro/vite/services/ssr/assets/oauth-mail.server-kN3UKU_L.js
var config = {
	google: {
...
function env(provider) {
	...
	if (!clientId || !clientSecret) throw new Error(`${provider === "google" ? "Google" : "Microsoft"} OAuth is not configured on the server.`);
	return {
		...item,
		clientId,
		clientSecret
...
function beginMailOAuth(provider, request) {
...
async function finishMailOAuth(provider, request) {
	const item = env(provider);
	const url = new URL(request.url);
	const origin = url.origin;
	const code = url.searchParams.get("code");
	const state = url.searchParams.get("state");
	const cookie = request.headers.get("cookie")?.match(new RegExp(`(?:^|; )gwero_oauth_${provider}=([^;]+)`))?.[1];
	if (!code || !state || !cookie || state !== cookie) return redirectResult(origin, "error", "OAuth state check failed");
	...
	const profileResponse = await fetch(item.profile, { headers: { authorization: `Bearer ${tokens.access_token}` } });
	...
	if (!email) return redirectResult(origin, "error", "The account did not provide an email address");
	const { supabaseAdmin } = await import("./client.server-DZIQA0ow.mjs");
	const storedProvider = provider === "google" ? "gmail" : "outlook";
	const { data: mailbox, error } = await supabaseAdmin.from("mailboxes").upsert({
		name: profile.name || email,
		from_email: email,
		from_name: profile.name || null,
		provider: storedProvider,
		last_status: "Connected with OAuth"
	}, { onConflict: "from_email" }).select().single();
	if (error || !mailbox) return redirectResult(origin, "error", error?.message || "Could not save mailbox");
...
function redirectResult(origin, key, value) {
	return new Response(null, {
		status: 302,
		headers: {
			location: `${origin}/mailboxes?${key}=${encodeURIComponent(value)}`,
			"set-cookie": "gwero_oauth_google=; Path=/; HttpOnly; Max-Age=0, gwero_oauth_microsoft=; Path=/; HttpOnly; Max-Age=0"
		}
	});
}
//#endregion
export { beginMailOAuth, finishMailOAuth };
