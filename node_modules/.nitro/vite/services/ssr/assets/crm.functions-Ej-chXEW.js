...
import { n as htmlToText, r as personalize } from "./personalize-BPwaxYCo.js";
...
//#endregion
//#region src/lib/crm.functions.ts?tss-serverfn-split
function appOrigin() {
...
var testMailbox = createServerFn({ method: "POST" }).validator((data) => data).handler(testMailbox_createServerFn_handler, async ({ data }) => {
	...
	if (mailbox.provider === "smtp") {
		...
		const { smtpVerify } = await import("./smtp.server-jw_Ubbfl.js");
		await smtpVerify({
		...
		if (mailbox.imap_host) {
			const { imapVerify } = await import("./imap.server-mEz2XBuS.js");
			await imapVerify({
...
var sendCampaign = createServerFn({ method: "POST" }).validator((data) => data).handler(sendCampaign_createServerFn_handler, async ({ data }) => {
	...
	const { buildMime } = await import("./mime.server-CiENyIT4.js");
	let sent = 0;
	let failed = 0;
	for (const recipient of recipients) {
		...
		try {
			...
			if (mailbox.provider === "smtp") {
				const { smtpSend } = await import("./smtp.server-jw_Ubbfl.js");
				await smtpSend({
					host: mailbox.smtp_host ?? "",
					port: mailbox.smtp_port ?? 587,
					secure: mailbox.smtp_secure,
	...
	for (const mailbox of mailboxes) {
		...
		try {
			...
			if (mailbox.provider === "smtp") {
				...
				const { imapFetchRecent } = await import("./imap.server-mEz2XBuS.js");
