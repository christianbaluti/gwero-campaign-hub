...
export async function saveAttachment(campaignId: string, file: File) {
  ...
  const directory = join(root, campaignId); await mkdir(directory, { recursive: true });
  const path = `${campaignId}/${randomUUID()}`;
  await writeFile(join(root, path), Buffer.from(await file.arrayBuffer()), { flag: "wx", mode: 0o600 });
...
export async function readAttachment(path: string) {
  ...
  if (parts.length !== 2 || !parts.every((part) => uuid.test(part))) throw new Error("Invalid attachment path");
